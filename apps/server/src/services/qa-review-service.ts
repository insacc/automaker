...
export class QAReviewService {
  ...
  private async runReviewer(
    ...
    const prompt = buildReviewerPrompt(diff, featureDescription, specification, iteration, previousIssues);
  ...
  private async postGitHubComment(
    ...
    try {
      ...
      console.error('[QAReview] Failed to post GitHub comment:', error);
  ...
  private async commitAndPushFixes(worktreePath: string, iteration: number): Promise<void> {
    try {
      ...
      if (status.trim()) {
        ...
        await execAsync(`git commit -m "fix: address QA review feedback (iteration ${iteration})"`, {
          cwd: worktreePath,
        });
